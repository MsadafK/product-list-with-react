...
import {
  LayoutGrid, List, Plus, Search, ChevronLeft, ChevronRight,
  MoreHorizontal, Package, Pencil, Trash2, ArrowUpDown,
  DollarSign, AlertTriangle, RefreshCw, Download, LogIn,
} from "lucide-react"
...
function ProductDashboard() {
  ...
  // Guard — agar login nahi hai toh write actions block karo
    ...
    try {
      ...
      if (lowStock.length > 0) {
        toast.warning(
          ...
          { description: lowStock.map((p) => `${p.name} (${p.stock} left)`).join(", "), duration: 6000 }
        )
      }
    } catch (err) {
  ...
  useMemo(() => { setCurrentPage(1) }, [debouncedSearchTerm, selectedCategory])
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchesCategory = selectedCategory === ALL_FILTER_LABEL || p.category === selectedCategory
  ...
  const handleAddOrEdit = async (productData) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        setProducts(products.map((p) => p.id === updated.id ? { ...updated, image: updated.image_url || "" } : p))
  ...
  const toggleSelectAll = () => setSelectedIds(selectedIds.length === currentProducts.length ? [] : currentProducts.map((p) => p.id))
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(selectedIds.map((id) => {
        ...
        return logActivity("deleted", p?.name || "Unknown", "Bulk delete")
      }))
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-5 md:space-y-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <Button variant="outline" size="sm" onClick={fetchProducts} disabled={isLoading} className="text-muted-foreground">
              <RefreshCw className={cn("w-4 h-4 sm:mr-2", isLoading && "animate-spin")} />
              <span className="hidden sm:inline">Refresh</span>
            </Button>
            <Button variant="outline" size="sm" onClick={handleExportCSV} disabled={isLoading || products.length === 0} className="text-muted-foreground">
              <Download className="w-4 h-4 sm:mr-2" />
              <span className="hidden sm:inline">Export</span>
            </Button>
            <Button size="sm" onClick={() => { if (!requireAuth()) return; setEditingProduct(null); setIsFormOpen(true) }} className="flex-1 sm:flex-none" disabled={isLoading}>
          ...
          <div className="px-3 sm:px-6 py-4 sm:py-5">
            ...
            <div className="flex items-end justify-between">
              <div className="flex items-baseline gap-1.5">
                ...
                {stats.lowStockCount > 0 && <span className="hidden sm:inline text-xs font-medium text-orange-500">needs attention</span>}
                {stats.lowStockCount === 0 && <span className="hidden sm:inline text-xs font-medium text-emerald-500">all good</span>}
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-2 text-muted-foreground flex-1 sm:flex-none">
                    ...
                    <span className="hidden sm:inline">
                      {sortBy === "default" ? "Sort" : sortBy === "name-asc" ? "Name A→Z" : sortBy === "name-desc" ? "Name Z→A" : sortBy === "price-asc" ? "Price ↑" : sortBy === "price-desc" ? "Price ↓" : sortBy === "stock-asc" ? "Stock ↑" : "Stock ↓"}
                    </span>
              ...
              <div className="flex items-center gap-1 bg-secondary p-1 rounded-lg">
                <Button variant={view === "list" ? "secondary" : "ghost"} size="sm" onClick={() => setView("list")} className={cn("px-2 sm:px-3", view === "list" && "bg-background shadow-sm")}>
                  <List className="w-4 h-4" /><span className="hidden sm:inline ml-2">List</span>
                </Button>
                <Button variant={view === "card" ? "secondary" : "ghost"} size="sm" onClick={() => setView("card")} className={cn("px-2 sm:px-3", view === "card" && "bg-background shadow-sm")}>
                  <LayoutGrid className="w-4 h-4" /><span className="hidden sm:inline ml-2">Card</span>
                </Button>
              </div>
            </div>
          </div>
        ...
        {view === "list" && selectedIds.length > 0 && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 py-2.5 rounded-lg bg-secondary border border-border">
            <p className="text-sm font-medium">{selectedIds.length} product{selectedIds.length > 1 ? "s" : ""} selected</p>
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={clearSelection} className="text-muted-foreground">Deselect all</Button>
        ...
        <main>
          {isLoading ? (
            <div className={view === "card" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6" : "border border-border rounded-xl overflow-hidden bg-card divide-y divide-border"}>
                ...
                <Card key={product.id} className="group overflow-hidden border-border bg-card transition-all hover:border-muted-foreground/50 flex flex-col">
                  ...
                  <CardContent className="space-y-3 px-4 pb-4">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border">
                      ...
                      <div className={cn("text-xs px-2 py-1 rounded-full font-medium", product.stock > 10 ? "bg-emerald-500/10 text-emerald-500" : "bg-orange-500/10 text-orange-500")}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left min-w-[600px]">
                  ...
                  <tbody className="divide-y divide-border">
                    {currentProducts.map((product) => (
                      <tr key={product.id} className={cn("hover:bg-secondary/20 transition-colors", selectedIds.includes(product.id) && "bg-secondary/30")}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="h-9 w-9 rounded border border-border overflow-hidden bg-secondary/50 shrink-0">
                              <img src={product.image || "/placeholder.svg?height=80&width=80"} alt={product.name} className="h-full w-full object-cover" />
                            </div>
                            <div>
                              <div className="font-medium text-foreground">{product.name}</div>
                              <div className="text-xs text-muted-foreground truncate max-w-[160px]">{product.description}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="inline-flex items-center rounded-md bg-secondary px-2 py-1 text-xs font-medium border border-border">{product.category}</span>
                        </td>
                        <td className="px-4 py-3 font-medium">${product.price.toFixed(2)}</td>
                        <td className="px-4 py-3">
                          <span className={cn("text-xs font-medium", product.stock > 10 ? "text-emerald-500" : "text-orange-500")}>
                            {product.stock} units
                          </span>
                        </td>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between py-4 gap-3 border-t border-border">
            <p className="text-xs sm:text-sm text-muted-foreground order-2 sm:order-1">
              ...
              <span className="text-foreground font-medium">{Math.min(currentPage * itemsPerPage, sortedProducts.length)}</span>{" "}
              of <span className="text-foreground font-medium">{sortedProducts.length}</span>
            </p>
            <div className="flex items-center gap-1 order-1 sm:order-2">
              <Button variant="outline" size="icon" className="h-8 w-8 bg-transparent" onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1}>
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <div className="flex items-center gap-1 mx-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                  <Button key={pageNum} variant={currentPage === pageNum ? "default" : "outline"} size="icon" className="h-8 w-8" onClick={() => setCurrentPage(pageNum)}>
                    {pageNum}
                  </Button>
                ))}
              </div>
              <Button variant="outline" size="icon" className="h-8 w-8 bg-transparent" onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>
      ...
      {/* Demo mode banner */}
      {!isSignedIn && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-3 rounded-xl border border-border bg-card shadow-lg">
          <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" />
          <p className="text-sm text-muted-foreground">
            You're viewing a <span className="font-medium text-foreground">read-only demo</span>
          </p>
          <Button size="sm" variant="outline" onClick={() => router.push("/sign-in")} className="ml-2 gap-1.5 shrink-0">
            <LogIn className="w-3.5 h-3.5" /> Sign In
          </Button>
        </div>
      )}

      {/* Demo sign-in prompt dialog */}
      <Dialog open={showDemoDialog} onOpenChange={setShowDemoDialog}>
        <DialogContent className="sm:max-w-[380px] bg-card border-border text-center">
          <DialogHeader>
            <DialogTitle className="text-center">Sign in to continue</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">
            You're in demo mode. Create an account or sign in to add, edit, or delete products.
          </p>
          <div className="flex flex-col gap-2 pt-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => setShowDemoDialog(false)} className="text-muted-foreground">
              Continue browsing
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-[500px] bg-card border-border">
          ...
          <ProductForm onSubmit={handleAddOrEdit} initialData={editingProduct} onCancel={() => setIsFormOpen(false)} isSubmitting={isSubmitting} />
        </DialogContent>
      </Dialog>
